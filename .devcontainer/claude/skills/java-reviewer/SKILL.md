...
description: |
  ...
  WHEN NOT: Non-Java code, non-Java projects, normal code requests, non-review requests
---
...
- Null safety (also with annotations)
- Stream/Collection patterns
- Modern Java features (17+)
- Benchmarks implemented correctly
multiSelect: true
```
...
public List<User> findUsers(final String query) {
...
### MEDIUM: Missing final and Null Safety
| Pattern                            | Issue               | Severity |
|------------------------------------|---------------------|----------|
| Non-final parameters               | Mutable state risk  | MEDIUM   |
| No @Nullable/@NotNull on parameter | Nullability unclear | MEDIUM   |
| No @Nullable/@NotNull on return    | Nullability unclear | MEDIUM   |

```java
// BAD: Non-final parameter
public void processData(Data data) {  // 'data' can be reassigned
    data = modify(data);
    // ...
}

// GOOD: Final parameter
public void processData(final Data data) {
    Data modified = modify(data);
    // ...
}

// BAD: No nullability annotations
public User findUser(String id) {
    return repository.findById(id);  // May return null
}

// GOOD: With nullability annotations
@Nullable
public User findUser(@NotNull final String id) {
    return
        repository.findById(id);  // Clearly may return null
}
```
...
public class User {
    public boolean equals(final Object obj) {  // Typo not caught
        // ...
    }
}
...
3. **Null Safety**: @Nullable/@NotNull
